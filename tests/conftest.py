...
from solana.wallet.router_api import DefaultAioHttpClient, SolanaWalletRouterAPI, AsyncSolanaWalletRouterAPI
from solana.wallet.router_api._utils import is_dict
...
logging.getLogger("solana.wallet.router_api").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[SolanaWalletRouterAPI]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with SolanaWalletRouterAPI(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncSolanaWalletRouterAPI]:
