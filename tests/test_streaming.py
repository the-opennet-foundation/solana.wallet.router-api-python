...
from solana.wallet.router_api._streaming import Stream, AsyncStream, ServerSentEvent


@pytest.mark.asyncio
@pytest.mark.parametrize("sync", [True, False], ids=["sync", "async"])
async def test_basic(sync: bool, client: SolanaWalletRouterAPI, async_client: AsyncSolanaWalletRouterAPI) -> None:
...
async def test_data_missing_event(
    sync: bool, client: SolanaWalletRouterAPI, async_client: AsyncSolanaWalletRouterAPI
) -> None:
    def body() -> Iterator[bytes]:
        yield b'data: {"foo":true}\n'
        yield b"\n"
